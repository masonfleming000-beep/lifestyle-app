...
import { normalizeProject, normalizeProjectCardDisplay } from "./careerProjectPresentation";
...
export function initCareerInformationPage(config: CareerInformationClientConfig) {
  ...
  function normalizeData(raw) {
    ...
    const normalized = {
      ...
      projects: normalizeArray(parsed.projects).map((item, index) => {
        ...
        return {
          ...normalized,
          id: normalized.id || makeId("project"),
    ...
    return formShell(
      ...
      `
        <div class="dynamic-form-grid">
          ...
          <label class="edu-label"><span>Project cover image</span><input id="dynamic-project-coverImage" class="form-input" placeholder="https://..." /></label>
          <label class="edu-label"><span>Link</span><input id="dynamic-project-link" class="form-input" placeholder="https://..." /></label>
          <div class="dynamic-form-full dynamic-project-card-options">
            <p class="field-hint custom-field-helper">Choose what shows on the project card in the public portfolio section.</p>
            <div class="dynamic-project-card-option-grid">
              <label class="check-row"><input id="dynamic-project-card-cover" type="checkbox" checked /><span>Show cover image</span></label>
              <label class="check-row"><input id="dynamic-project-card-subtitle" type="checkbox" checked /><span>Show subtitle</span></label>
              <label class="check-row"><input id="dynamic-project-card-description" type="checkbox" checked /><span>Show description</span></label>
              <label class="check-row"><input id="dynamic-project-card-skills" type="checkbox" checked /><span>Show skills</span></label>
              <label class="check-row"><input id="dynamic-project-card-link" type="checkbox" checked /><span>Show project link</span></label>
            </div>
          </div>
  ...
  function bindDynamicFormActions() {
    ...
    document.getElementById("dynamic-save-project-btn")?.addEventListener("click", async () => {
      ...
      data.projects.unshift(normalizeProject({
        ...
        coverImage: getValue("dynamic-project-coverImage"),
        link: getValue("dynamic-project-link"),
        visible: getChecked("dynamic-project-visible"),
        cardDisplay: normalizeProjectCardDisplay({
          cover: getChecked("dynamic-project-card-cover"),
          subtitle: getChecked("dynamic-project-card-subtitle"),
          description: getChecked("dynamic-project-card-description"),
          skills: getChecked("dynamic-project-card-skills"),
          link: getChecked("dynamic-project-card-link"),
        }),
  ...
  function buildSavedCard(group, item) {
    ...
    if (group === "projects") {
      ...
      return `${top}${item.coverImage ? `<div class="saved-project-cover-wrap"><img src="${escapeHtml(item.coverImage)}" alt="${escapeHtml(item.title || "Project cover")}" class="saved-project-cover" /></div>` : ""}<h3 class="card-title">${escapeHtml(item.title || "Project")}</h3><p><strong>Subtitle:</strong> ${escapeHtml(item.subtitle || "—")}</p><p>${escapeHtml(item.description || "—")}</p><p><strong>Skills:</strong> ${escapeHtml(item.skills || "—")}</p>${item.link ? `<p><strong>Link:</strong> <a href="${escapeHtml(item.link)}" target="_blank" rel="noreferrer">${escapeHtml(item.link)}</a></p>` : ""}<p><strong>Card display:</strong> ${[cardDisplay.cover && "Cover", cardDisplay.subtitle && "Subtitle", cardDisplay.description && "Description", cardDisplay.skills && "Skills", cardDisplay.link && "Link"].filter(Boolean).join(", ") || "Title only"}</p>${extra}`;
    }
