...
export const GET: APIRoute = async ({ request }) => {
  ...
  if (!parsed.hostname.includes("nutritionvalue.org")) {
    return new Response(
      JSON.stringify({ error: "Only nutritionvalue.org allowed" }),
      {
        status: 400,
        headers: { "content-type": "application/json" },
      }
  ...
  try {
    ...
    if (!upstream.ok) {
      return new Response(
        JSON.stringify({
          error: "Upstream detail fetch failed",
    ...
    const fat = extractFirstNumber(
      [/Total Fat\s+(\d+(?:\.\d+)?)\s*g/i, /Fat\s+(\d+(?:\.\d+)?)\s*g/i],
      text
    );

    const protein = extractFirstNumber([/Protein\s+(\d+(?:\.\d+)?)\s*g/i], text);
    ...
    const addedSugars = extractNutrientAmount(
    ...
    const saturatedFat = extractNutrientAmount(
