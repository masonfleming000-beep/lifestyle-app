...
type ThemeSurface = { color?: string; texture?: string };
type ThemeComponents = Record<string, { surface?: string }>;
...
export function initPublicProfilePage(config: PublicProfileClientConfig) {
  ...
  const username = config.username || pageRoot?.getAttribute("data-username") || "";
      ...
      function normalizeThemeMode(mode) {
        return mode === "light" || mode === "dark" || mode === "custom" ? mode : "custom";
      }

      function applySurfaceColor(root, name, surface, fallbackColor) {
        root.style.setProperty(`--${name}-color`, surface?.color || fallbackColor);
      }

      function applyPublicTheme(profile) {
        const rootEl = document.documentElement;
        const theme = safeObject(profile?.theme);
        const surfaces = safeObject(theme?.surfaces);
        const components = safeObject(theme?.components);
        ...
        const fallbackDark = {
          header: { color: "#0f172a", texture: "solid" },
          section: { color: "#1e293b", texture: "solid" },
          background: { color: "#020617", texture: "solid" },
          dropdown: { color: "#111827", texture: "solid" },
        };

        const effectiveSurfaces = mode === "dark" ? fallbackDark : {
          header: { ...fallbackLight.header, ...safeObject(surfaces.header) },
          section: { ...fallbackLight.section, ...safeObject(surfaces.section) },
          background: { ...fallbackLight.background, ...safeObject(surfaces.background) },
          dropdown: { ...fallbackLight.dropdown, ...safeObject(surfaces.dropdown) },
        };

        rootEl.dataset.theme = mode;
        rootEl.dataset.textSize = profile?.theme?.textSize || "md";

        rootEl.dataset.headerTexture = effectiveSurfaces.header.texture || "solid";
        rootEl.dataset.sectionTexture = effectiveSurfaces.section.texture || "solid";
        rootEl.dataset.backgroundTexture = effectiveSurfaces.background.texture || "solid";
        rootEl.dataset.dropdownTexture = effectiveSurfaces.dropdown.texture || "solid";

        rootEl.dataset.navbarSurface = safeObject(components.navbar).surface || "header";
        rootEl.dataset.heroSurface = safeObject(components.hero).surface || "header";
        rootEl.dataset.sectionCardSurface = safeObject(components.sectionCard).surface || "section";
        rootEl.dataset.quickLinkCardSurface = safeObject(components.quickLinkCard).surface || "section";
        rootEl.dataset.quoteCardSurface = safeObject(components.quoteCard).surface || "section";
        rootEl.dataset.statCardSurface = safeObject(components.statCard).surface || "section";
        rootEl.dataset.cardSurface = safeObject(components.card).surface || "section";
        rootEl.dataset.dropdownCardSurface = safeObject(components.dropdownCard).surface || "dropdown";
        rootEl.dataset.embedSurface = safeObject(components.embed).surface || "section";
        rootEl.dataset.metricSurface = safeObject(components.metric).surface || "section";
        rootEl.dataset.inputSurface = safeObject(components.input).surface || "section";

        applySurfaceColor(rootEl, "header", effectiveSurfaces.header, fallbackLight.header.color);
        applySurfaceColor(rootEl, "section", effectiveSurfaces.section, fallbackLight.section.color);
        applySurfaceColor(rootEl, "background", effectiveSurfaces.background, fallbackLight.background.color);
        applySurfaceColor(rootEl, "dropdown", effectiveSurfaces.dropdown, fallbackLight.dropdown.color);
      }
      ...
      async function init() {
        ...
        try {
          ...
          applyPublicTheme(currentProfile);
