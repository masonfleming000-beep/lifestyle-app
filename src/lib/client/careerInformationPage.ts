...
export function initCareerInformationPage(config: CareerInformationClientConfig) {
  ...
  async function uploadCareerAsset(file, kind = "file") {
    ...
    const payload = await res.json().catch(() => null);
    if (!res.ok) throw new Error(payload?.error || `Upload failed (${res.status})`);
    if (!payload?.fileUrl) throw new Error("Upload succeeded but no file URL was returned.");
    return payload;
  }

  let data = normalizeData(cloneDefaults());
  let editingState = { group: "", id: "" };
  let hasLoadedInitialState = false;

  function setEditingItem(group, id = "") {
    editingState = { group: String(group || ""), id: String(id || "") };
  }

  function clearEditingItem(group = "") {
    if (!group || editingState.group === String(group)) {
      editingState = { group: "", id: "" };
    }
  }

  function getEditingItem(group, fallback = {}) {
    const normalizedGroup = String(group || "");
    if (editingState.group === normalizedGroup && editingState.id) {
      const match = normalizeArray(data?.[normalizedGroup]).find((item) => String(item?.id || "") === editingState.id);
      if (match) return match;
    }
    if (["profile", "contact", "resume"].includes(normalizedGroup)) {
      const singleton = normalizeArray(data?.[normalizedGroup])[0];
      if (singleton && typeof singleton === "object") return singleton;
    }
    return fallback;
  }

  function isEditingGroup(group) {
    return editingState.group === String(group || "") && !!editingState.id;
  }

  function upsertSectionItem(group, item, options = {}) {
    const normalizedGroup = String(group || "");
    const list = normalizeArray(data?.[normalizedGroup]);
    const normalizedItem = { ...item, id: item?.id || makeId(normalizedGroup) };

    if (options.singleton) {
      data[normalizedGroup] = [normalizedItem];
      return normalizedItem;
    }

    const existingIndex = list.findIndex((entry) => String(entry?.id || "") === String(normalizedItem.id || ""));
    if (existingIndex >= 0) list.splice(existingIndex, 1, normalizedItem);
    else list.unshift(normalizedItem);
    data[normalizedGroup] = list;
    return normalizedItem;
  }
  ...
  function formShell(helper, body, buttonId, buttonText, options = {}) {
    ...
    return `
      <div class="dynamic-form-card">
        ...
        ${editing ? `<div class="selector-state selected">Editing saved entry</div>` : ""}
        ${body}
        <div class="assignment-actions">
          ${buttonId ? `<button class="button-primary career-inline-button" id="${buttonId}" type="button">${escapeHtml(buttonText)}</button>` : ""}
          ${editing ? `<button class="button-secondary career-inline-button" type="button" data-action="cancel-entry-edit" data-edit-group="${escapeHtml(group)}">Cancel edit</button>` : ""}
        </div>
      </div>
    `;
  }
  ...
  function buildProfileForm() {
    ...
    return formShell(
      ...
      "Save Profile Basics",
      { group: "profile", editing: isEditingGroup("profile") }
    );
  }

  function buildExternalLinksForm() {
    const item = getEditingItem("externalLinks", {});
    return formShell(
      ...
      isEditingGroup("externalLinks") ? "Save External Link" : "Add External Link",
      { group: "externalLinks", editing: isEditingGroup("externalLinks") }
    );
  }

  function buildExperienceForm() {
    const item = getEditingItem("experience", {});
    return formShell(
      ...
      { group: "experience", editing: isEditingGroup("experience") }
    );
  }

  function buildLeadershipForm() {
    const item = getEditingItem("leadership", {});
    return formShell(
      ...
      { group: "leadership", editing: isEditingGroup("leadership") }
    );
  }

  function buildProjectsForm() {
    const item = getEditingItem("projects", {});
    return formShell(
      ...
      "Save Project",
      { group: "projects", editing: isEditingGroup("projects") }
    );
  }

  function buildOrganizationsForm() {
    const item = getEditingItem("organizations", {});
    return formShell(
      ...
      "Save Organization",
      { group: "organizations", editing: isEditingGroup("organizations") }
    );
  }

  function buildHonorsForm() {
    const item = getEditingItem("honors", {});
    return formShell(
      ...
      "Save Honor or Award",
      { group: "honors", editing: isEditingGroup("honors") }
    );
  }

  function buildLicensesForm() {
    const item = getEditingItem("licenses", {});
    return formShell(
      ...
      { group: "licenses", editing: isEditingGroup("licenses") }
    );
  }
  ...
  function buildContactForm() {
    ...
    return formShell(
      ...
      { group: "contact", editing: isEditingGroup("contact") }
    );
  }
  ...
  function buildResumeForm() {
    ...
    return formShell(
      ...
      "Upload + Save Resume",
      { group: "resume", editing: isEditingGroup("resume") }
    );
  }

  function buildSchoolForm() {
    const item = getEditingItem("school", {});
    return formShell(
      ...
      { group: "school", editing: isEditingGroup("school") }
    );
  }
  ...
  function buildSimpleTextForm(prefix, defaults) {
    ...
    return formShell(
      ...
      defaults.buttonText,
      { group: prefix, editing: isEditingGroup(prefix) }
    );
  }

  function buildTimelineForm() {
    const item = getEditingItem("timelineItems", {});
    return formShell(
      "Legacy section kept for compatibility with existing saved content.",
      `
        <div class="dynamic-form-grid">
      ...
      "Save Timeline Item",
      { group: "timelineItems", editing: isEditingGroup("timelineItems") }
    );
  }

  function buildRecommendationsForm() {
    const item = getEditingItem("recommendations", {});
    return formShell(
      "Legacy section kept for compatibility with existing saved content.",
      `
        <div class="dynamic-form-grid one">
      ...
      "Save Recommendation",
      { group: "recommendations", editing: isEditingGroup("recommendations") }
    );
  }

  function buildStarForm() {
    const item = getEditingItem("star", {});
    return formShell(
      "Legacy section kept for compatibility with existing saved content.",
      `
        <div class="dynamic-form-grid one">
      ...
      "Save STAR Example",
      { group: "star", editing: isEditingGroup("star") }
    );
  }
  ...
  function populateEditingForm(group) {
    const item = getEditingItem(group, null);
    if (!item || !isEditingGroup(group)) return;

    if (group === "externalLinks") {
      setFieldValue("dynamic-link-type", item.type || "website");
      setFieldValue("dynamic-link-label", item.label || "");
      setFieldValue("dynamic-link-url", item.url || "");
      setFieldChecked("dynamic-link-visible", item.visible);
    } else if (group === "experience") {
      setFieldValue("dynamic-exp-role", item.role || "");
      setFieldValue("dynamic-exp-company", item.company || "");
      setFieldValue("dynamic-exp-location", item.location || "");
      setFieldValue("dynamic-exp-startDate", item.startDate || "");
      setFieldValue("dynamic-exp-endDate", item.endDate || "");
      setFieldValue("dynamic-exp-summary", item.summary || "");
      setFieldValue("dynamic-exp-bullets", normalizeArray(item.bullets).join("\n"));
      setFieldChecked("dynamic-exp-visible", item.visible);
    } else if (group === "leadership") {
      setFieldValue("dynamic-leadership-title", item.title || "");
      setFieldValue("dynamic-leadership-organization", item.organization || "");
      setFieldValue("dynamic-leadership-date", item.date || "");
      setFieldValue("dynamic-leadership-summary", item.summary || "");
      setFieldValue("dynamic-leadership-bullets", normalizeArray(item.bullets).join("\n"));
      setFieldChecked("dynamic-leadership-visible", item.visible);
    } else if (group === "projects") {
      setFieldValue("dynamic-project-title", item.title || "");
      setFieldValue("dynamic-project-subtitle", item.subtitle || "");
      setFieldValue("dynamic-project-description", item.description || "");
      setFieldValue("dynamic-project-skills", item.skills || "");
      setFieldValue("dynamic-project-link", item.link || "");
      const display = normalizeProjectCardDisplay(item.cardDisplay);
      setFieldChecked("dynamic-project-card-showCoverPhoto", display.showCoverPhoto);
      setFieldChecked("dynamic-project-card-showSubtitle", display.showSubtitle);
      setFieldChecked("dynamic-project-card-showDescription", display.showDescription);
      setFieldChecked("dynamic-project-card-showSkills", display.showSkills);
      setFieldChecked("dynamic-project-card-showLink", display.showLink);
      setFieldChecked("dynamic-project-visible", item.visible);
    } else if (group === "organizations") {
      setFieldValue("dynamic-organization-name", item.name || "");
      setFieldValue("dynamic-organization-role", item.role || "");
      setFieldValue("dynamic-organization-date", item.date || "");
      setFieldValue("dynamic-organization-description", item.description || "");
      setFieldChecked("dynamic-organization-visible", item.visible);
    } else if (group === "honors") {
      setFieldValue("dynamic-honor-title", item.title || "");
      setFieldValue("dynamic-honor-value", item.value || "");
      setFieldValue("dynamic-honor-issuer", item.issuer || "");
      setFieldValue("dynamic-honor-date", item.date || "");
      setFieldValue("dynamic-honor-description", item.description || "");
      setFieldChecked("dynamic-honor-visible", item.visible);
    } else if (group === "licenses") {
      setFieldValue("dynamic-license-title", item.title || "");
      setFieldValue("dynamic-license-issuer", item.issuer || "");
      setFieldValue("dynamic-license-date", item.date || "");
      setFieldValue("dynamic-license-credentialId", item.credentialId || "");
      setFieldValue("dynamic-license-link", item.link || "");
      setFieldChecked("dynamic-license-visible", item.visible);
    } else if (group === "school") {
      setFieldValue("dynamic-school-title", item.title || "");
      setFieldValue("dynamic-school-prof", item.prof || "");
      setFieldValue("dynamic-school-stage", item.stage || "");
      setFieldValue("dynamic-school-helped", item.helped || "");
      setFieldValue("dynamic-school-relevance", item.relevance || "");
      setFieldValue("dynamic-school-notes", item.notes || "");
      setFieldChecked("dynamic-school-visible", item.visible);
    } else if (["about", "looking", "pitch"].includes(group)) {
      setFieldValue(`dynamic-${group}-title`, item.title || "");
      setFieldValue(`dynamic-${group}-body`, item.body || "");
      setFieldChecked(`dynamic-${group}-visible`, item.visible);
    } else if (group === "timelineItems") {
      setFieldValue("dynamic-timeline-title", item.title || "");
      setFieldValue("dynamic-timeline-date", item.date || "");
      setFieldValue("dynamic-timeline-description", item.description || "");
      setFieldChecked("dynamic-timeline-visible", item.visible);
    } else if (group === "recommendations") {
      setFieldValue("dynamic-rec-title", item.title || "");
      setFieldValue("dynamic-rec-body", item.body || "");
      setFieldValue("dynamic-rec-owner", item.owner || "");
      setFieldChecked("dynamic-rec-visible", item.visible);
    } else if (group === "star") {
      setFieldValue("dynamic-star-title", item.title || "");
      setFieldValue("dynamic-star-situation", item.situation || "");
      setFieldValue("dynamic-star-task", item.task || "");
      setFieldValue("dynamic-star-action", item.action || "");
      setFieldValue("dynamic-star-result", item.result || "");
      setFieldChecked("dynamic-star-visible", item.visible);
    }
  }
  ...
  function renderDynamicForm() {
    ...
    populateEditingForm(select.value);
  }
  ...
  function bindDynamicFormActions() {
    document.getElementById("dynamic-save-profile-btn")?.addEventListener("click", async () => {
      try {
        ...
        upsertSectionItem("profile", {
          id: getEditingItem("profile", {})?.id || data.profile?.[0]?.id || makeId("profile"),
    ...
    document.getElementById("dynamic-save-link-btn")?.addEventListener("click", async () => {
      ...
      if (!url) return;
      const current = getEditingItem("externalLinks", {});
      upsertSectionItem("externalLinks", {
        id: current.id || makeId("link"),
      ...
      clearEditingItem("externalLinks");
      await persistAndRefresh();
      renderDynamicForm();
    });
    ...
    document.getElementById("dynamic-save-exp-btn")?.addEventListener("click", async () => {
      ...
      if (!role) return;
      const current = getEditingItem("experience", {});
      upsertSectionItem("experience", {
        id: current.id || makeId("experience"),
        role,
    ...
    document.getElementById("dynamic-save-leadership-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("leadership", {});
      upsertSectionItem("leadership", {
        id: current.id || makeId("leadership"),
        title,
    ...
    document.getElementById("dynamic-save-project-btn")?.addEventListener("click", async () => {
      ...
      try {
        ...
        const current = getEditingItem("projects", {});
        const project = {
          id: current.id || makeId("project"),
          title,
        ...
        upsertSectionItem("projects", {
          ...project,
          projectPageSections: syncProjectPageSections({ ...current, ...project, projectPageSections: current.projectPageSections || [] }),
        });
        clearEditingItem("projects");
        await persistAndRefresh();
        renderDynamicForm();
      } catch (error) {
    ...
    document.getElementById("dynamic-save-organization-btn")?.addEventListener("click", async () => {
      ...
      if (!name) return;
      const current = getEditingItem("organizations", {});
      upsertSectionItem("organizations", {
        id: current.id || makeId("organization"),
        name,
      ...
      clearEditingItem("organizations");
      await persistAndRefresh();
      renderDynamicForm();
    });
    ...
    document.getElementById("dynamic-save-honor-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("honors", {});
      upsertSectionItem("honors", {
        id: current.id || makeId("honor"),
        title,
    ...
    document.getElementById("dynamic-save-license-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("licenses", {});
      upsertSectionItem("licenses", {
        id: current.id || makeId("license"),
        title,
    ...
    document.getElementById("dynamic-save-contact-btn")?.addEventListener("click", async () => {
      ...
      if (!value) return;
      upsertSectionItem("contact", {
        id: getEditingItem("contact", {})?.id || data.contact?.[0]?.id || makeId("contact"),
    ...
    document.getElementById("dynamic-save-resume-btn")?.addEventListener("click", async () => {
      ...
      try {
        ...
        upsertSectionItem("resume", nextResume, { singleton: true });
        clearEditingItem("resume");
        await persistAndRefresh();
        renderDynamicForm();
      } catch (error) {
    ...
    document.getElementById("dynamic-save-school-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("school", {});
      upsertSectionItem("school", {
        id: current.id || makeId("school"),
        title,
    ...
    document.getElementById("dynamic-save-timeline-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("timelineItems", {});
      upsertSectionItem("timelineItems", {
        id: current.id || makeId("timeline"),
        title,
      ...
      clearEditingItem("timelineItems");
      await persistAndRefresh();
      renderDynamicForm();
    });
    ...
    document.getElementById("dynamic-save-rec-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("recommendations", {});
      upsertSectionItem("recommendations", {
        id: current.id || makeId("recommendation"),
        title,
      ...
      clearEditingItem("recommendations");
      await persistAndRefresh();
      renderDynamicForm();
    });
    ...
    document.getElementById("dynamic-save-star-btn")?.addEventListener("click", async () => {
      ...
      if (!title) return;
      const current = getEditingItem("star", {});
      upsertSectionItem("star", {
        id: current.id || makeId("star"),
        title,
      ...
      clearEditingItem("star");
      await persistAndRefresh();
      renderDynamicForm();
    });

    document.querySelectorAll('[data-action="cancel-entry-edit"]').forEach((button) => {
      button.addEventListener("click", () => {
        clearEditingItem(button.getAttribute("data-edit-group") || "");
        renderDynamicForm();
      });
    });
  }
  ...
  function renderGroup(wrapId, countId, group, items, emptyText) {
    ...
    items.forEach((item) => {
      ...
      card.querySelector(".edit-entry-btn")?.addEventListener("click", () => {
        const selector = document.getElementById("dynamic-section-select");
        if (selector) selector.value = group;
        setEditingItem(group, item.id);
        renderDynamicForm();
      ...
      card.querySelector(".danger-btn")?.addEventListener("click", async () => {
        if (editingState.group === group && editingState.id === item.id) {
          clearEditingItem(group);
          renderDynamicForm();
        }
